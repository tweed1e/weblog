...
name_time_gender <- name_time %>% 
  filter(who_en != "") %>%
  filter(grepl("(Mr\\.|Mrs\\.|Miss|Ms\\.)", x = who_en)) %>%
...
Whoops. There's an obvious break in the series for Count---so something happened to the data collection or the structure of the hansard (it's less likely that parliament itself changed significantly at that time). On the other hand, the frequency doesn't change, so it's unlikely to change our conclusions for gender imbalance. Just one thing to keep in mind for the future.
